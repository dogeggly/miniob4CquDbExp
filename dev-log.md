...
```bash
# 在 Docker 容器的 /workspace/miniob 目录下运行测试
...
1. **向量长度上限与页面大小的关系**: VECTOR(16383) 在记录中占用 65532 字节，可能超过默认页面大小，需要在实际使用中注意
...
# 四、IVF Flat 向量索引近似搜索实现日志

## 概述

在 MiniOB 中实现了 IVF (Inverted File) Flat 索引，支持近似向量距离搜索：
- **CREATE VECTOR INDEX 语法**：支持 `CREATE VECTOR INDEX ... WITH (distance=..., type=ivfflat, lists=N, probes=M)`
- **IVF Flat 索引实现**：K-Means 聚类训练、倒排列表存储、ANN 近似搜索
- **LIMIT 子句**：支持 `LIMIT N` 关键字，在排序结果中截断返回 Top-N 条记录
- **优化器下推**：优化器智能探测 IVF Flat 索引，将 TableScan 替换为 VECTOR_INDEX_SCAN 物理算子
...
```
IvfflatIndex
  ├── centroids_ (K-Means 聚类中心)
  ├── inverted_lists_ (倒排列表: 簇 ID → [向量 + RID])
  ├── lists_ (聚类数)
  ├── probes_ (搜索探测数)
  └── distance_method_ (距离度量)
```
...
ProjectPhysicalOperator
  |
LimitPhysicalOperator (LIMIT N)
  |
OrderByPhysicalOperator (ORDER BY)
  |
VectorIndexScanPhysicalOperator (ANN 搜索 + 候选获取)
```

## 改动文件清单

### 1. `src/observer/storage/index/index_meta.h` — 修改
- 新增 `IndexType` 枚举（`BPLUS_TREE`/`IVFFLAT`）和 `index_type_name()` 辅助函数
- `IndexMeta` 新增字段：`index_type_`、`distance_method_`、`lists_`、`probes_`
- 新增 `init(name, field, type)` 重载方法
- 新增访问器：`index_type()`、`distance_method()`、`lists()`、`probes()` 及其 setter

### 2. `src/observer/storage/index/index_meta.cpp` — 修改
- 新增 JSON 序列化字段：`index_type`、`distance_method`、`lists`、`probes`
- `init()` 默认调用 BPLUS_TREE 类型的 init 重载
- `from_json()` 支持向后兼容（无 index_type 字段时默认 BPLUS_TREE）

### 3. `src/observer/storage/index/ivfflat_index.h` — 重写
- 完整的 `IvfflatIndex` 类声明（继承 `Index`）
- `is_vector_index()` 返回 `true`
- 新增 `ann_search(query, limit)` 方法
- 新增 `IvfflatIndexScanner` 类（`IndexScanner` 子类）
- 新增 `IvfflatListItem` 结构体

### 4. `src/observer/storage/index/ivfflat_index.cpp` — 新建
- **`create()`**: 扫描表中所有记录 → K-Means 训练 → 构建倒排列表 → 持久化
- **`open()`**: 从磁盘文件加载质心和倒排列表
- **`insert_entry()`**: 提取向量 → 找最近质心 → 加入倒排列表
- **`delete_entry()`**: 按 RID 查找并删除
- **`ann_search()`**: 找最近 probes 个质心 → 扫描对应列表 → 精确距离排序 → 返回 Top-K RID
- **`train_kmeans()`**: 随机初始化质心 → 迭代分配-更新直至收敛（max 20 轮）
- **`save_to_file()`/`load_from_file()`**: 二进制格式持久化
- **`IvfflatIndexScanner`**: 实现 `IndexScanner` 接口，ANN 扫描逻辑

距离计算方法（与 `VectorType` 静态方法对应）：
| 方法 | 公式 | 实现 |
|------|------|------|
| `cosine` | `1 - (a·b)/(\|a\|\|b\|)` | 内联计算 |
| `dot` | `-sum(a[i]*b[i])` | 内联计算 |
| `euclidean`/`l2` | `sum((a[i]-b[i])^2)` | 内联计算 |

### 5. `src/observer/sql/parser/lex_sql.l` — 修改
- 新增 token：`WITH`、`LIMIT`

### 6. `src/observer/sql/parser/yacc_sql.y` — 修改
- 新增 token 声明：`WITH`、`LIMIT`
- 新增 `create_vector_index_stmt` 语法规则，支持两种形式：
  - `CREATE VECTOR INDEX idx ON t (col)`
  - `CREATE VECTOR INDEX idx ON t (col) WITH (distance=cosine, type=ivfflat, lists=10, probes=3)`
- 新增 `with_option_list` 规则（支持 ID=ID / ID=NUMBER / ID=FLOAT 键值对）
- 新增 `create_vector_index_stmt` 到 `command_wrapper`
- `select_stmt` 新增 `opt_limit` 子句
- 新增 `opt_limit` 规则（`LIMIT NUMBER` 或空 → -1）
- 新增 `%type` 声明

### 7. `src/observer/sql/parser/parse_defs.h` — 修改
- `SqlCommandFlag` 枚举新增 `SCF_CREATE_VECTOR_INDEX`
- `SelectSqlNode` 新增 `limit_num` 字段（默认 -1）
- `CreateIndexSqlNode` 新增向量索引字段：`index_type_str`、`distance_method`、`lists`、`probes`

### 8. `src/observer/sql/stmt/stmt.h` — 修改
- `DEFINE_ENUM` 新增 `CREATE_VECTOR_INDEX`

### 9. `src/observer/sql/stmt/stmt.cpp` — 修改
- `create_stmt()` 新增 `SCF_CREATE_VECTOR_INDEX` 分派（复用到 `CreateIndexStmt::create()`）
- `stmt_type_ddl()` 新增 `CREATE_VECTOR_INDEX` 为 DDL 类型

### 10. `src/observer/sql/stmt/create_index_stmt.h` — 修改
- 新增向量索引属性：`is_vector_index_`、`index_type_str_`、`distance_method_`、`lists_`、`probes_`
- `type()` 根据 `is_vector_index_` 返回 `CREATE_VECTOR_INDEX` 或 `CREATE_INDEX`
- 新增 `set_vector_options()` 方法和访问器

### 11. `src/observer/sql/stmt/create_index_stmt.cpp` — 修改
- `create()` 中传递向量索引选项到 `CreateIndexStmt`

### 12. `src/observer/sql/stmt/select_stmt.h` — 修改
- 新增 `limit_num_` 成员和 `limit_num()` 访问器

### 13. `src/observer/sql/stmt/select_stmt.cpp` — 修改
- `create()` 中传递 `limit_num` 到 `SelectStmt`

### 14. `src/observer/sql/executor/create_index_executor.cpp` — 修改
- assert 接受 `CREATE_VECTOR_INDEX` 或 `CREATE_INDEX` 类型
- 根据 `is_vector_index()` 调用 `table->create_vector_index()` 或 `table->create_index()`

### 15. `src/observer/sql/executor/command_executor.cpp` — 修改
- switch-case 新增 `CREATE_VECTOR_INDEX`（与 `CREATE_INDEX` 共用 `CreateIndexExecutor`）

### 16. `src/observer/storage/table/table.h` — 修改
- 新增 `create_vector_index()` 方法声明

### 17. `src/observer/storage/table/table.cpp` — 修改
- 新增 `create_vector_index()` 实现（委托到 `engine_->create_vector_index()`）

### 18. `src/observer/storage/table/table_engine.h` — 修改
- 新增纯虚方法 `create_vector_index()`

### 19. `src/observer/storage/table/heap_table_engine.h` — 修改
- 新增 `create_vector_index()` 重写声明

### 20. `src/observer/storage/table/heap_table_engine.cpp` — 修改
- 包含 `ivfflat_index.h`
- 实现 `create_vector_index()`:
  1. 校验字段类型为 VECTOR
  2. 创建 `IndexMeta`（类型 IVFFLAT，设置距离方法/lists/probes）
  3. 创建 `IvfflatIndex` 并调用 `create()`（含 K-Means 训练）
  4. 更新 `indexes_` 列表和元数据持久化

### 21. `src/observer/storage/table/lsm_table_engine.h` — 修改
- 新增 `create_vector_index()` stub（返回 `RC::UNIMPLEMENTED`）

### 22. `src/observer/sql/operator/logical_operator.h` — 修改
- `LogicalOperatorType` 新增 `LIMIT`、`VECTOR_INDEX_SCAN`

### 23. `src/observer/sql/operator/physical_operator.h` — 修改
- `PhysicalOperatorType` 新增 `LIMIT`、`VECTOR_INDEX_SCAN`

### 24. `src/observer/sql/operator/limit_logical_operator.h` — 新建
- `LimitLogicalOperator` 类：存储 `limit_num_`

### 25. `src/observer/sql/operator/limit_logical_operator.cpp` — 新建
- 构造函数

### 26. `src/observer/sql/operator/limit_physical_operator.h` — 新建
- `LimitPhysicalOperator` 类：
  - `open()`: 打开子算子
  - `next()`: 从子算子拉取，计数到 `limit_num` 后返回 `RECORD_EOF`
  - `current_tuple()`: 透传子算子的元组

### 27. `src/observer/sql/operator/limit_physical_operator.cpp` — 新建
- 实现截断逻辑：跟踪已返回行数 `count_`，达限返回 EOF

### 28. `src/observer/sql/operator/vector_index_scan_physical_operator.h` — 新建
- `VectorIndexScanPhysicalOperator` 类：
  - 存储查询向量 `query_vector_`、limit、谓词列表
  - `open()` 调用 `IvfflatIndex::ann_search()` 获取候选 RID
  - `next()` 逐个通过 RID 获取记录并应用谓词过滤

### 29. `src/observer/sql/operator/vector_index_scan_physical_operator.cpp` — 新建
- `open()`: 调用 `IvfflatIndex::ann_search()` 获取候选 RID 列表，设置 row tuple schema
- `next()`: 通过 RID 获取记录 → 应用谓词过滤 → 返回匹配行
- `filter()`: 遍历谓词表达式，AND 逻辑求值

### 30. `src/observer/sql/optimizer/logical_plan_generator.cpp` — 修改
- 包含 `limit_logical_operator.h`
- `create_plan(SelectStmt*)`: 在 ORDER BY 和 PROJECT 之间插入 LIMIT 逻辑算子（当 `limit_num > 0` 时）

### 31. `src/observer/sql/optimizer/physical_plan_generator.h` — 修改
- 前向声明 `LimitLogicalOperator`
- 新增 `create_plan(LimitLogicalOperator&, ...)` 声明

### 32. `src/observer/sql/optimizer/physical_plan_generator.cpp` — 修改
- 包含 `limit_logical_operator.h`、`limit_physical_operator.h`、`vector_index_scan_physical_operator.h`
- `create()` 分派新增 `case LogicalOperatorType::LIMIT`
- `create_plan(TableGetLogicalOperator&)`: 新增向量索引检测逻辑：
  1. **检查向量索引**：在谓词中寻找 `DISTANCE` 函数 → 提取向量字段和查询向量 → 查找 IVF 索引 → 创建 `VectorIndexScanPhysicalOperator`
  2. 在 B+ 树索引检查之前执行，优先匹配向量索引
- 新增 `create_plan(LimitLogicalOperator&)`: 递归创建子算子 → 创建 `LimitPhysicalOperator`

### 33. `src/observer/sql/operator/logical_operator.cpp` — 修改
- `can_generate_vectorized_operator()` 新增 `LIMIT`、`VECTOR_INDEX_SCAN` 返回 false

## 功能支持范围

| 功能 | 支持情况 |
|------|---------|
| `CREATE VECTOR INDEX ... WITH (type=ivfflat, ...)` | ✅ |
| K-Means 聚类训练 | ✅ |
| 倒排列表构建 | ✅ |
| ANN 近似搜索（probes 簇选择） | ✅ |
| 距离度量：cosine / dot / euclidean / l2 | ✅ |
| 索引持久化（二进制格式） | ✅ |
| `LIMIT N` 语法解析 | ✅ |
| LIMIT 截断执行 | ✅ |
| 优化器探测 IVF 索引替换 TableScan | ✅ |
| VECTOR_INDEX_SCAN 物理算子 | ✅ |
| 与 ORDER BY 联合使用 | ✅ |
| 与传统 B+ 树索引共存 | ✅ |
| LSM 引擎支持 | ❌ 返回 UNIMPLEMENTED |

## 测试

功能测试脚本位于 `test/vector_index_test.sh`，共 40 条用例，覆盖以下场景：

| 分组 | 用例数 | 覆盖内容 |
|------|--------|---------|
| 1. 数据准备 | 8 | VECTOR(3) 建表、5 行向量插入、VECTOR(4) 跨维度测试表 |
| 2. CREATE VECTOR INDEX 语法 | 12 | 基本语法、distance=euclidean/cosine/dot/l2、lists/probes 单选项、完整组合选项、非 VECTOR 列报错、重复索引名报错、不存在的表/列报错 |
| 3. 精确距离查询 | 3 | cosine/euclidean 全表扫描、完全匹配验证 |
| 4. LIMIT 语法 | 4 | LIMIT 1/3/0、带 WHERE 的 LIMIT |
| 5. DISTANCE + ORDER BY + LIMIT | 6 | Top-N 余弦/欧几里得/点积/l2 距离排序截断、LIMIT 等于/超过全表行数 |
| 6. LIMIT 边界和错误 | 2 | LIMIT 后跟标识符报错、LIMIT 负数报错 |
| 7. AS 别名 + ORDER BY + LIMIT | 2 | 距离别名排序截断、混合别名过滤排序 |
| 8. 跨维度错误 | 1 | VECTOR(3) 列 vs VECTOR(4) 查询向量报错 |
| 9. B+ 树索引兼容 | 2 | ID 列 B+ 树索引正常创建、等值查询正常返回 |
...
**测试断言策略**：
- `success` — 响应含 `SUCCESS` 或 `|`（多列表格输出）
- `failure` — 响应含 `FAILURE` 或 `> ` 错误格式
- `contains:X` — 响应含指定子串（用于单列输出无 `|` 的场景）

## 已知问题 / 待完成

1. **LSM 引擎不支持**：`LsmTableEngine::create_vector_index()` 返回 `RC::UNIMPLEMENTED`
2. **CLI 模式下 limit 不传递**：VectorIndexScanPhysicalOperator 默认使用 limit=100 作为 ANN 搜索候选数
3. **质心固定不更新**：当前实现中插入新数据不会触发质心重新训练
4. **内存索引**：倒排列表完全加载在内存中，大数据量时可能导致内存不足
5. **无并发控制**：索引操作没有加锁保护
